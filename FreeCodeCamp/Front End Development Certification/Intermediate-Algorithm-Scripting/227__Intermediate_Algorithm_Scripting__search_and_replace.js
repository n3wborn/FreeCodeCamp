...
function myReplace(str, before, after) {
    // check if <before> is upper case
    if (before.charAt(0) === before.charAt(0).toUpperCase()) {
        myRegex = after.replace(after.charAt(0), after.charAt(0).toUpperCase());
        return str.replace(before, myRegex);
    } else {
        myStr = str.replace(before, after);
        return myStr;
    }
}
